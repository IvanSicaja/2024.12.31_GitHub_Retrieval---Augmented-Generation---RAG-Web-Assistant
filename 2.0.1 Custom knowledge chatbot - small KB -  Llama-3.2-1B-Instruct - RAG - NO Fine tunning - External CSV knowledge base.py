...
def generate_response(query, use_conversation_history=True):
    ...
    response = generator(
        ...
        # max_length = 512,  # (int, optional, defaults to 20) — The maximum length the generated tokens can have.
        ...
        early_stopping = True,  # (bool or str, optional, defaults to False) — Stops generation earlier if all beams meet a stopping condition (e.g., generating an end-of-sequence token) before reaching the maximum token length.
        ...
        top_k = 5,  # (int, optional, defaults to 50) — The number of highest probability vocabulary tokens to keep for top-k-filtering.
        top_p = 0.9,  # (float, optional, defaults to 1.0) — Keeps the smallest set of tokens with probabilities that add up to top_p or higher.
        ...
        repetition_penalty = 1.0,  # (float, optional, defaults to 1.0) — The parameter for repetition penalty. 1.0 means no penalty.
